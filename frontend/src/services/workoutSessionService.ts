...
export interface SetLog { id: string; set_number: number; target_reps_min: number; target_reps_max: number; actual_reps: number | null; load_kg: number | null; rpe: number | null; notes: string; completed: boolean; completed_at?: string | null; target_rest_seconds: number | null; actual_rest_seconds: number | null; is_extra: boolean; is_warmup?: boolean; skipped_at: string | null; skip_reason: string | null; previous_performance?: PreviousSetPerformance | null; }
...
export interface WorkoutSession { id: string; training_date: string; workout_label: string; session_kind: WorkoutSessionKind; status: "active" | "paused" | "completed"; notes: string; session_rpe?: number | null; session_quality?: number | null; post_workout_discomfort?: boolean; profile_id: string | null; profile_name: string | null; applied_restrictions: ProfileRestriction[]; exercises: ExerciseLog[]; }

export interface WorkoutCheckout {
  sessionRpe: number;
...
async function loadDetails(session: Omit<WorkoutSession, "exercises">): Promise<WorkoutSession> {
  ...
  for (const exercise of exercises ?? []) {
    const { data: sets, error: setsError } = await supabase.from("set_logs").select("id, set_number, target_reps_min, target_reps_max, actual_reps, load_kg, rpe, notes, completed, completed_at, target_rest_seconds, actual_rest_seconds, is_extra, is_warmup, skipped_at, skip_reason").eq("exercise_log_id", exercise.id).order("is_warmup", { ascending: false }).order("set_number");
...
async function startOrLoadWorkoutOnce(userId: string, date: string, label: string, sessionKind: WorkoutSessionKind): Promise<WorkoutSession> {
  const supabase = getSupabaseClient();
  const projection = "id, training_date, workout_label, session_kind, status, notes, session_rpe, session_quality, post_workout_discomfort, profile_id, profile_name, applied_restrictions";
...
export async function loadExistingWorkout(userId: string, date: string, sessionKind: WorkoutSessionKind = "real"): Promise<WorkoutSession | null> {
  const supabase = getSupabaseClient();
  const { data, error } = await supabase.from("workout_sessions").select("id, training_date, workout_label, session_kind, status, notes, session_rpe, session_quality, post_workout_discomfort, profile_id, profile_name, applied_restrictions").eq("user_id", userId).eq("training_date", date).eq("session_kind", sessionKind).maybeSingle();
  if (error) throw error;
  if (!data) return null;
...
export async function updateSession(id: string, status: WorkoutSession["status"], notes: string, checkout?: WorkoutCheckout) {
  const now = new Date().toISOString();
  const { error } = await getSupabaseClient().from("workout_sessions").update({
    status,
    notes,
    paused_at: status === "paused" ? now : null,
    completed_at: status === "completed" ? now : null,
    ...(checkout ? {
      session_rpe: checkout.sessionRpe,
      session_quality: checkout.sessionQuality,
      post_workout_discomfort: checkout.discomfort,
    } : {}),
  }).eq("id", id);
  if (error) throw error;
}

export async function finishWorkoutWithPending(id: string, notes: string, reason: string, checkout?: WorkoutCheckout): Promise<number> {
  const supabase = getSupabaseClient();
  if (checkout) {
    const { error: checkoutError } = await supabase.from("workout_sessions").update({
      session_rpe: checkout.sessionRpe,
      session_quality: checkout.sessionQuality,
      post_workout_discomfort: checkout.discomfort,
    }).eq("id", id);
  ...
  const { data, error } = await supabase.rpc("finish_workout_with_pending", {
    p_session_id: id,
    p_notes: notes,
