...
import { createManualWorkout, loadExistingWorkout, previewAutomaticWorkout, replaceUnstartedWorkout, type WorkoutSession } from "../services/workoutSessionService";
import type { WorkoutExerciseTemplate } from "../lib/workoutTemplates";

type SetupMode = "choice" | "preview" | "manual" | "existing" | "locked";
...
export default function WorkoutSetupPage() {
  ...
  useEffect(() => {
    ...
    void Promise.all([loadExerciseCatalog(), loadActiveProfileContext(user.id, date), isExerciseCatalogAdmin(user.id), loadExistingWorkout(user.id, date)])
      ...
      .catch(() => setMessage("Não foi possível carregar a preparação do treino."));
  }, [date, user]);
