...
export default function UserAdminPage(){
  ...
  return <main className="admin-shell">
    ...
    {deleteTarget&&<div className="confirmation-backdrop"><form className="confirmation-dialog admin-delete-user-dialog" role="dialog" aria-modal="true" aria-labelledby="delete-user-title" onSubmit={event=>{event.preventDefault();void confirmDelete();}}><span className="setup-status setup-status--locked">AÇÃO IRREVERSÍVEL</span><h2 id="delete-user-title">Excluir usuário?</h2><p>Esta ação remove a conta <strong>{deleteTarget.email}</strong> e pode apagar seus dados vinculados. Para confirmar, digite o e-mail completo.</p><label>E-mail de confirmação<input autoFocus value={deleteConfirmation} onChange={event=>setDeleteConfirmation(event.target.value)}/></label>{deleteError&&<p className="admin-delete-user-error" role="alert">{deleteError}</p>}<div className="admin-delete-user-actions"><button type="button" disabled={busy} onClick={()=>setDeleteTarget(null)}>Cancelar</button><button className="danger-action" type="submit" disabled={busy||deleteConfirmation.trim().toLowerCase()!==deleteTarget.email.toLowerCase()}>{busy?"Excluindo…":"Excluir definitivamente"}</button></div></form></div>}
  </main>;
}
