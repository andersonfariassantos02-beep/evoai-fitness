...
import { chartCoordinates, evolutionChange, type ExerciseEvolution } from "../lib/exerciseEvolution";
...
export default function ExerciseEvolutionPanel({ exercises }: { exercises: ExerciseEvolution[] }) {
  const [exerciseKey, setExerciseKey] = useState(exercises[0]?.key ?? "");
  const [metric, setMetric] = useState<Metric>("loadKg");
  useEffect(() => {
    if (!exercises.some((exercise) => exercise.key === exerciseKey)) setExerciseKey(exercises[0]?.key ?? "");
  ...
  const variation = exercise ? evolutionChange(exercise.points, metric) : null;
  ...
  return <section className="exercise-evolution" aria-labelledby="exercise-evolution-title">
    <header>
      ...
      {exercises.length > 0 && <label>Exercício<select aria-label="Exercício do gráfico" value={exercise?.key ?? ""} onChange={(event) => setExerciseKey(event.target.value)}>
    ...
    {exercise && <>
      ...
      <div className="evolution-summary"><div><span>Valor mais recente</span><strong>{latest?.toLocaleString("pt-BR")} <small>{definition.unit}</small></strong></div><div><span>Variação no período</span><strong className={(variation ?? 0) >= 0 ? "positive" : "negative"}>{variation === null ? "Referência inicial" : `${variation > 0 ? "+" : ""}${variation.toLocaleString("pt-BR")}%`}</strong></div><div><span>Sessões registradas</span><strong>{exercise.points.length}</strong></div></div>
